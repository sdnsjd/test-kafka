...
## Конфиг вашего коннектора

```json
{
  "id": "kafka-connector-read",
  ...
  "settings": {
    ...
    "parameters": [
      ...
      { "key": "sasl.mechanism",            "value": "SCRAM-SHA-512" },
      {
        "key": "sasl.jaas.config",
        "value": "org.apache.kafka.common.security.scram.ScramLoginModule required username=\"appuser\" password=\"appuser-secret\";"
      },
      {
        "key": "ssl.truststore.location",
        "value": "./certs/cloudstudio.truststore.jks"
      },
      { "key": "ssl.truststore.password",  "value": "changeit" },
      { "key": "max.poll.records",          "value": "10" }
    ]
  }
}
```

> `ssl.truststore.location` должен указывать на путь, **откуда запускается ваша платформа**.  
> Если запускаете из корня `kafka-kraft/`, то путь будет `./certs/cloudstudio.truststore.jks`.
